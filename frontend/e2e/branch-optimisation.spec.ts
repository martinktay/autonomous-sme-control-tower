/**
 * E2E tests for the Branch Optimisation page.
...
test.describe("Branch Optimisation Page", () => {
  test("redirects to login when unauthenticated", async ({ page }) => {
    await page.goto("/branch-optimisation");
    await expect(page).toHaveURL("/login");
  });

  test("loads with branch optimisation heading when authenticated", async ({ page }) => {
    await seedAuth(page);
    await page.goto("/branch-optimisation");
    await expect(page.locator("h1")).toContainText("Cross-Branch Optimisation");
  });

  test("shows AI optimise button", async ({ page }) => {
    await seedAuth(page);
    await page.goto("/branch-optimisation");
    await expect(page.getByRole("button", { name: /ai optimise/i })).toBeVisible();
  });
});
