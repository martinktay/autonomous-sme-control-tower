/**
 * E2E tests for the Predictions page.
...
test.describe("Predictions Page", () => {
  ...
  test("loads with predictions heading when authenticated", async ({ page }) => {
    await seedAuth(page);
    await page.goto("/predictions");
    await expect(page.locator("h1")).toContainText("Inventory Predictions");
  });

  test("shows AI forecast button", async ({ page }) => {
    await seedAuth(page);
    await page.goto("/predictions");
    await expect(page.getByRole("button", { name: /ai forecast/i })).toBeVisible();
  });
});
