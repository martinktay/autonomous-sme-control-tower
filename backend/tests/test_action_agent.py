...
class TestWorkflowExecution:
    ...
    def test_execute_strategy_success(self, mock_bedrock):
        mock_client = Mock()
        mock_client.invoke_nova_lite.return_value = '{"status": "success", "target": "INV-001", "actions_taken": ["collected"], "summary": "done"}'
    ...
    def test_execute_strategy_generates_unique_id(self, mock_bedrock):
        mock_client = Mock()
        mock_client.invoke_nova_lite.return_value = '{"target": "X"}'
        mock_bedrock.return_value = mock_client
    ...
    @patch("app.agents.action_agent.get_bedrock_client")
    def test_execute_strategy_includes_org_in_prompt(self, mock_bedrock):
        mock_client = Mock()
        mock_client.invoke_nova_lite.return_value = '{"target": "X"}'
        mock_bedrock.return_value = mock_client
        ...
        assert "org_test_99" in prompt_arg
        assert "Collect invoices" in prompt_arg

    @patch("app.agents.action_agent.get_bedrock_client")
    def test_execute_strategy_failure_does_not_raise(self, mock_bedrock):
        """Failures should be captured, not raised"""
        mock_client = Mock()
        mock_client.invoke_nova_lite.side_effect = RuntimeError("boom")
        mock_bedrock.return_value = mock_client
